...
import agree from "@/assets/images/contact/agree.png";
import arrow from "@/assets/images/contact/arrowTopRight.png";
import refuse from "@/assets/images/contact/refuse.png";
  ...
  return (
    <Spin spinning={loading}>
      <div className="mb-2 flex cursor-pointer flex-row items-center justify-between bg-gradient-to-r from-[rgba(255,255,255,0.39)] from-0% to-[rgba(230,230,230,0.1521)] to-100% py-[22px] pl-8 pr-12 transition-colors hover:bg-[#F3F3F3]">
        <div className="mr-[138px] flex flex-row">
          <OIMAvatar
            src={getAvatarUrl()}
            text={getTitle()}
          ...
          <div className="ml-6">
            ...
            <p className="line-clamp-4 text-xs text-[#737373]">
              {t("application.information")}: {source.reqMsg}
            </p>
          </div>
        </div>
...
const STATUS_ICON_MAP: Partial<Record<ApplicationHandleResult, string>> = {
  [ApplicationHandleResult.Agree]: agree,
  [ApplicationHandleResult.Reject]: refuse,
};
...
const StatusIcon: React.FC<{
  ...
  const iconSrc = STATUS_ICON_MAP[status!] || arrow;

  return <img className="mr-2 h-4 w-4" src={iconSrc} alt="" />;
};
